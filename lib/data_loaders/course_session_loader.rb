  ...
  module Bot
    class CourseSessionLoader < Teachbase::Bot::DataLoaderController
      ...
      def delete_all_by(options)
        appshell.user.find_by_type(:cs, options).destroy_all
      end

      def delete_unsigned(lms_tb_ids)
        db_tb_ids = appshell.user.find_by_type(:cs, status: status, account_id: appshell.current_account.id, scenario: category)
      ...
      def courses_db_with_paginate
        appshell.user.find_by_type(:cs, status: status, account_id: appshell.current_account.id, limit: limit, offset: offset,
