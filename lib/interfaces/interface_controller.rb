  ...
  module Bot
    class InterfaceController
      ...
      def button_sign(cont_type, object)
        "#{attach_emoji(cont_type)} #{object.name} #{attach_emoji(object.status)}"
      end

      def attachments(object)
        result = ["#{Emoji.t(:bookmark_tabs)} <i>#{I18n.t('attachments').capitalize}</i>"]
      ...
      def comments(object)
        result = ["#{Emoji.t(:lips)} <i>#{I18n.t('comments').capitalize}</i>"]
      ...
      def answers
        ...
        entity.answers.order(created_at: :desc).each do |answer|
          ...
          result << "<b>#{I18n.t('answer').capitalize} №#{answer.attempt}. #{I18n.t('state').capitalize}: #{attach_emoji(answer.status)} <i>#{I18n.t(answer.status).capitalize}</i></b>
      ...
      def description
        msg = "<pre>#{entity.description}</pre>\n\n"
        msg = "#{msg}#{attachments(entity)}" if entity.attachments?
        msg
      end
